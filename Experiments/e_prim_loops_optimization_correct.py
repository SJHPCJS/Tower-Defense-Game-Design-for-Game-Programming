...
BATCH_DIR = os.path.join(OUT_DIR, "prim_loops_optimization_correct_batch")
...
def algo_prim_loops():
    """Prim's algorithm with loops - generates maze with random loop addition"""
    ...
    for y in range(HEIGHT):
        for x in range(WIDTH):
            if (grid[y][x] == 1 and 
                ...
                sum(grid[ny][nx] == 0 for nx, ny in neighbors4(x, y)) >= 2):
                grid[y][x] = 0
...
def optimize_prim_loops(grid: List[List[int]]) -> Tuple[List[List[int]], int]:
    """Optimize the prim loops by removing useless path tiles"""
...
def run_experiment():
    """Run the corrected prim loops optimization experiment"""
    print("Running Prim Loops Optimization Experiment (Correct Version)...")
    ...
    for i in range(RUNS_PER_ALGO):
        ...
        original_grid = algo_prim_loops()
        generation_time = time.perf_counter() - start_time
        
        # Optimize the map
        start_time = time.perf_counter()
        optimized_grid, cleaned = optimize_prim_loops(original_grid)
    ...
    # Save CSV results
    csv_path = os.path.join(OUT_DIR, "prim_loops_optimization_results.csv")
...
def generate_charts(df, original_stats, optimized_stats, cleaned_counts):
    ...
    plt.title("Prim Loops Path Tiles Count Comparison")
    for bar, val in zip(bars, path_counts):
        plt.text(bar.get_x()+bar.get_width()/2, val+0.5, f"{val:.1f}",
                 ha="center", va="bottom", fontsize=10)
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURES_DIR, "prim_loops_path_tiles_comparison.png"), dpi=300)
    plt.close()
    ...
    plt.title("Prim Loops Distribution of Cleaned Tiles")
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURES_DIR, "prim_loops_cleaned_tiles_distribution.png"), dpi=300)
    plt.close()
